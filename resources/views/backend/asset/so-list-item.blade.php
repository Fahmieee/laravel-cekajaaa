...
$url_callback = url('admin/stock_opname_asset/list-item/'.$id);
$status = tv($id,'stock_opname_asset','status');
?>
<p>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="{{url('admin/stock_opname_asset/list-item/'.$id)}}"><i class="fa fa-th-list"></i> Scanned Item</a></li>
    <li><a href="{{url('admin/stock_opname_asset/total-peritem/'.$id)}}"><i class="fa fa-th-list"></i> Total PerItem</a></li>
</ul>
    ...
    <table class="table table-hover table-striped table-bordered">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{$item->code}}</td>
          <td>{{tv($item->id_cms_users,'cms_users','name')}}</td>
          <td>{{$item->created_at}}</td>
          @if($status != 'PUBLISH')
            <td align="center">
              <a href="{{action('AdminStockOpnameAssetController@getDeleteItem').'/'.$item->id.'?id_stock_opname_asset='.$id.'&url_callback='.$url_callback}}" onclick="return confirm('Are you sure for delete item?')" class="btn btn-sm btn-danger">
